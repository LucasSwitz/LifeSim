...
TEST_F(ComponentUserBaseTest, SingleComponent)
{
    ...
    bool contains_user_1 = (std::find(comps->begin(), comps->end(), user_1.get()) != comps->end());
    bool contains_user_2 = (std::find(comps->begin(), comps->end(), user_2.get()) != comps->end());
