...
class ResourceFactory
{
            ...
            if (is_regular_file(current_path))
            {
                ...
                if (file_read_stream.good())
                {
                    ...
                    if (preamble.IsValid() && preamble.IsType(_script_type))
                    {
                        ...
                        if (_configurable)
                        {
                            AddResource(preamble, Configure(current_path_string, preamble.GetFlag("Name")));
                        }
                        else
                        {
                            AddResource(preamble, current_path_string);
                        }
                    }
                    else
                    {
  ...
  protected:
    ...
    virtual void AddResource(Preamble &pre, T *scriptable_object){};
    virtual void AddResource(Preamble &pre, std::string script_path){};
    virtual void Reset() {}
