...
class StageEditor
{
  public:
    ...
    void Draw(ptr<Stage> stage)
    {
        ...
        if (_last_stage.get() != stage.get())
            _last_selection = -1;

        std::string name = "##LBV" + stage->GetName();
        ImGui::ListBoxVector(name.c_str(), &_selected_instance, _instance_names);
